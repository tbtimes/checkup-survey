...
class Survey(models.Model):
    ...
    form_chatter = models.TextField(
        help_text='Optional, all respondents read this before filling out form (you can set chatter for specific respondents when creating an assignment).',
        blank=True)
...
class Title(models.Model):
    ...
    order = models.PositiveSmallIntegerField(default=0,
                                             help_text='Lower order is a higher rank. So, president is 0 and village clerk is 50')
...
class Assignment(models.Model):
    ...
    display_chatter = models.TextField(help_text='Optional text the reader sees before reading respondent\'s answers.',
                                       blank=True)
    ...
    receipt_confirmed = models.BooleanField(default=False,
                                            help_text='When respondent has confirmed receipt, check this box.')
    survey_complete = models.BooleanField(default=False, editable=False,
                                          help_text='Has the respondent answered the questions currently assigned.')
    confirmation_sent = models.BooleanField(default=False,
                                            help_text='Check this after you have sent an email to respondents with their answers.')
    ...
    def tmpl(self):
        ...
        return select_template(
            ["%s/survey-%s-text.html" % (self.survey.home_slug, txt), "checkup/survey-%s-text.html" % txt]).name
...
class Contribution(models.Model):
    ...
    def __unicode__(self):
        return self.assignment.respondent.last_name + ' - ' + self.assignment.survey.__unicode__() + ': $' + str(
            self.amount)
